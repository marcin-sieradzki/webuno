<template>
  <div
    class="h-52 w-36 min-w-36 max-w-36 bg-gray-50 rounded-lg border-4"
    @click="playCard"
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    const { card, randomlyRotated, reversed, disabled, cards, absolute, type } =
      toRefs(props);
    ...
    const classObject = computed(() => {
      return {
        ...
        "transform translate-x-5": type.value === CardTypeEnum.hand,
    ...
    const buildTransform = () => {
      ...
      return `translate(${cardIndex * 2.5}rem, -${cardIndex * 13}rem)`;
    };
